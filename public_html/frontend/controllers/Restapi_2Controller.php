 ...
 */
class Restapi_2Controller extends Controller {
    ...
    public function actionCategories($id = 0) {
        ...
        "parent": 0,
        "image": {
            "src": "http://size.ir/frontend/web/sig_pic/banner.png"
        },
        "count": "2"
    }, {
        "id": 707,
        ...
        foreach ($post_to_show as $mother_cat) {
            ...
            $cats = CategoryHasCategory::find()->with('productCategory')->where('parent_category=' . $mother_cat->id)->all();
    ...
    public function actionProducts($category = 0) {
        ...
        for($p=1;$p<9;$p++){ 
            ...
            $temp['images'] = [["src" => 'http://size.ir/frontend/web/sig_pic/sig'.$p.'.jpg']];
            
               
            
            $temp['attributes'] = [
                //['name' => 'color','id'=>"dd", 'options' => ['red', 'blue', 'black']], 
                [   'name' => 'size',
                    'id'=>"dd",
                    'options' => ['S', 'M', 'L'],
                    ...
                    'options_2' => [
                        [
                            'value'=>'پاکت',
                            'price'=>'100',
                            'img'=>'http://size.ir/frontend/web/sig_pic/sig'.$p.'.jpg'
                        ],
                        [ 
                            'value'=>'باکس',
                            ...
                            'price'=>'200',
                            'sale'=>'180',
                            'img'=>'http://size.ir/frontend/web/sig_pic/sigbox.png'
                        ],
                        [
                            'value'=>'کارتن',
                            'price'=>'400',
                ...
                //['name' => 'وزن','id'=>"dd", 'options' => ['۱۰۰hh', 'M', 'L']]
...
// Echo the message.
    ...
    public function actionRegister() {
        ...
        $user = new User();
        $code = rand(1000, 9999);
        $user->name_and_fam = $_GET['first_name'] . " " . $_GET['last_name'];
        $user->username = $_GET['username'];
        $user->email = $_GET['email'];
        $user->code = $code;
        $user->setPassword($_GET['user_pass']);
        $user->generateAuthKey();
        $user->save();

        $send = new \sendAPI('lopen', '1qaz!QAZ');
        $mobiles = array($_GET['username']);
        $body = 'سلام ';
        $body .= 'کد فعال سازی   : ';
